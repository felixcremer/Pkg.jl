            ...
            if isfile(artifacts_toml)
                ...
                if isfile(hook_path)
                    mktempdir() do project
                        # We generate a Project.toml that explicitly allows access to a bare minimum
                        # of stdlibs.  A natural extension of this would be to include `Preferences`
                        # at some point in the future, perhaps if that becomes a standard library.
                        open(joinpath(project, "Project.toml"), "w") do io
                            TOML.print(io, Dict("deps" => Dict(
